...
extract_metadata <- function(file_path) {
  ...
  if (is.na(yaml_start) || is.na(yaml_end) || yaml_start >= yaml_end) {
    message("Skipping file (invalid YAML): ", file_path)
    return(NULL)
  }
  ...
  tryCatch({
    ...
    # determine challenge name from folder path (i.e., challenges/YYYY_WK_Title)
    challenge_dir <- basename(dirname(dirname(file_path)))
    ...
    result$categories_list <- list(
      if (!is.null(metadata$categories)) metadata$categories else character(0)
    )
    ...
    message("Metadata extraction failed: ", file_path)
    return(NULL)
  })
}

# find all .qmd files in r/ and python/ directories under challenges/
qmd_files <- list.files(
  ...
  grep("/r/|/python/", ., value = TRUE)

# extract metadata and bind if any valid rows
metadata_list <- map(qmd_files, extract_metadata)
valid_metadata <- keep(metadata_list, ~ !is.null(.x))

if (length(valid_metadata) > 0) {
  all_metadata <- bind_rows(valid_metadata) %>%
    filter(!is.na(title)) %>%
    distinct(relative_path, .keep_all = TRUE) %>%
    mutate(year = format(sort_date, "%Y")) %>%
    arrange(desc(sort_date))
} else {
  all_metadata <- tibble::tibble()
}
```
...
if (nrow(all_metadata) == 0) {
  ...
  for (current_year in years) {
    ...
    if (file.exists(image_path) || file.exists(abs_image_path)) {
      ...
      cat('<div style="width: 100%; height: 200px; display: flex; align-items: center; justify-content: center; overflow: hidden; margin-bottom: 1rem; background-color: #f5f3ef; border-radius: 6px;">\n')
      ...
      cat('<div style="background: linear-gradient(135deg, #7da492 0%, #b6c1a9 100%); height: 200px; display: flex; align-items: center; justify-content: center; color: #fdfdfd; font-weight: 600; text-align: center; border-radius: 6px; margin-bottom: 1rem;">\n')
      cat('<a href="', html_path, '" style="color: #fdfdfd; text-decoration: none;">', row$title, '</a>\n')
      cat('</div>\n\n')
    }
